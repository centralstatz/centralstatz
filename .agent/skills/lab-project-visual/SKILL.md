...
If generation is available, create a brief first, generate the image, inspect it, save it under `projects/<slug>/`, optimize it, add alt text, and label conceptual status where confusion is possible. Use the image-generation skill/tool when available.

If generation is unavailable, create `projects/<slug>/visual-brief.md` from `assets/visual-brief-template.md`, place a restrained placeholder or omit `image`, and explain the final asset path. Replacement must require only changing `image`.
